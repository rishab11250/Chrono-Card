...
describe('content and deterministic turns', () => {
  it('contains all cards, enemy kinds, and connected rooms with valid dimensions', () => {
    expect(Object.keys(CARDS)).toHaveLength(19);
...
describe('enemy rounds and expedition outcomes', () => {
  ...
  it('locks the exit until cleared, advances rooms, heals, and wins in the final room', () => {
    ...
    let choosing = applyAction(drafting, 'p1', {
      type: 'pick-relic',
      relicId: drafting.relicChoices!.p1[0],
    });
    choosing = applyAction(choosing, 'p1', {
      type: 'draft-card',
      cardId: choosing.draftChoices.p1[0],
    });
    ...
    expect(next.players[0].hp).toBeGreaterThanOrEqual(7);
  ...
  it('revives fallen allies but never players who abandoned the room', () => {
    ...
    while (choosing.phase === 'drafting') {
      const pId = activePlayer(choosing).id;
      if (choosing.relicChoices?.[pId]?.length) {
        choosing = applyAction(choosing, pId, {
          type: 'pick-relic',
          relicId: choosing.relicChoices[pId][0],
        });
      } else {
        choosing = applyAction(choosing, pId, {
          type: 'draft-card',
          cardId: choosing.draftChoices[pId][0],
        });
      }
    }
