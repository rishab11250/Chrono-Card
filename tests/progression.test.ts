...
import {ACTS,LEVELS,createGame,applyAction,abandonPlayer,levelIndex,activePlayer,type GameState} from '@chrono/shared';
function draftAll(s:GameState){while(s.phase==='drafting')s=applyAction(s,activePlayer(s).id,{type:'draft-card',cardId:s.draftChoices[activePlayer(s).id][0]});return s;}
...
describe('Act room graphs',()=>{
 ...
 it('offers both routes, validates ownership and rejects fabricated or repeated choices',()=>{
  const state=draftAll(atExit());expect(state.phase).toBe('choosing');expect(state.roomChoices).toEqual(['cinder','archive']);
 ...
 it('hands off an abandoned chooser without an extra enemy round',()=>{const state=draftAll(atExit());const next=abandonPlayer(state,'a');expect(next.phase).toBe('choosing');expect(next.round).toBe(state.round);expect(next.active).toBe(1);expect(applyAction(next,'b',{type:'choose-room',roomId:next.roomChoices[0]}).phase).toBe('playing');});
 it('crosses Acts through their declared entry, not the adjacent array index',()=>{const s=createGame('solo',[{id:'a',name:'A'}]);s.level=levelIndex(ACTS[0].rooms.at(-1)!.id);s.enemies=[];const room=LEVELS[s.level];const y=room.tiles.findIndex(row=>row.includes('E'));const x=room.tiles[y].indexOf('E');s.players[0].x=x-1;s.players[0].y=y;s.players[0].hand=['step1'];expect(draftAll(applyAction(s,'a',{type:'play',card:0,target:{x,y}})).level).toBe(levelIndex(ACTS[1].entry));});
});
